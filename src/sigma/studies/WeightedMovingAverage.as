package sigma.studies
{
	...
	public class WeightedMovingAverage
	{
		...
		public function contructing( value:Object , duration:String ):void
		{
			...
			_value.main['elements'][_chartIndex]['colour'] = "#ff1596";
		...
		public function addData(allData:Array , index:int):void
		{
			...
			if( _index >= _period - 1 )
			{
				var val:Number = getWSmaValue(index - (_period-1) , _allBaseChartData) ;
				var val_p:Number = getWSmaValue(index - (_period-1) , _allBaseChartData , "percentage" ) ;
		...
		public function updateDataAt(allData:Array , index:int):void
		{
			...
			if( _index >= _period - 1 )
			{
				...
				chartData.setItemAt({"timeStamp":data['TimeStamp1'],"value":val},index);
		...
		private function getWSmaValue(startIndex:int , allData:Array , property:String = "value" ):Number
		{
			var wsmaValue:Number = 0 ;
			var pos:int = 1 ;
			var facSum:int = 0 ;
			
			for ( var i:int=startIndex ; i < _period + startIndex ; i++)
			{
				wsmaValue = wsmaValue + ( allData[i][property] * pos ) ;
				facSum = facSum + pos ;				
				pos++;				
			}
			wsmaValue = wsmaValue / facSum ;
			return wsmaValue;			
		}
