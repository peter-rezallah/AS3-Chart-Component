...
import mx.rpc.events.HeaderEvent;
		...
		private function make_label( lblStyle:Object ):YTextField
		{
			...
			tf.width = 75;
			tf.height = 15;
			tf.text = replace_magic_values(lblStyle.val, lblStyle.pos);
		...
		private function replace_magic_values(labelText:String, yVal:Number):String {
			if( yVal > 99.00 ) labelText = labelText.replace('#val#', NumberUtils.formatNumber(Math.round(yVal)));
			else labelText = labelText.replace('#val#', NumberUtils.formatNumberYLabels4(yVal));
			return labelText;
		}
	}
}