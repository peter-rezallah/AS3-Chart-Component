	...
	public class Base extends Element
	{
		...
		protected function parse_value( props:Properties ):void {
			...
			if ( !props.has('value') ) {
				...
				props.set('value', Number.MIN_VALUE );
			}
		...
		protected function replace_magic_values( t:String ): String {
			...
			t = t.replace('#value#', NumberUtils.formatNumber( this.value ));
			t = t.replace('#val#', NumberUtils.formatNumber( this.top - this.value ));
		...
		protected function resize_helper( sc:ScreenCoords ):Object {
			...
			if( this.value == Number.MIN_VALUE )
				...
				bar_bottom = sc.get_y_from_val(this.value, this.right_axis);
